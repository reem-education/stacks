...
void GazeboRosP3D::UpdateChild()
{
  ...
  // XXX: we now publish a tf frame in addition to the pose topic
  // restore this condition to a proper state when the tf is published cleanly
  if (true || this->pub_.getNumSubscribers() > 0)
  {
